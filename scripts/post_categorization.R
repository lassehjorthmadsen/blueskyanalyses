...
posts_file  <- paste0("data/top_posts_", posts_date, ".csv")
pcats_file  <- posts_file |> str_replace("top_posts_", "top_posts_cat_")
...
# Chunk the data
posts_chunked <- split(unique_posts, ceiling(seq_len(nrow(unique_posts))/chunk_size))
...
# Set-up chat
chat <- chat_gemini(system_prompt = system_prompt, model = "gemini-2.5-flash")
